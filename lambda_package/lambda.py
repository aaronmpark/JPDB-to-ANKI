...
from aws_lambda_powertools.utilities.data_classes.api_gateway_proxy_event import event_source
...
def lambda_handler(event: APIGatewayProxyEvent, context):
    url = event.query_string_parameters.get("url")
    filename = event.query_string_parameters.get("filename", "jpdb_vocab.apkg")
    tmp_path = f"/tmp/{filename}"

    logger.info(f"Scraping vocab from {url}")
    vocab = scrape_vocab(url)
    logger.info(f"Creating Anki deck at {tmp_path}")
    create_anki_deck(vocab, tmp_path)
